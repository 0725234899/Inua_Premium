...
<body>
    ...
    $sql_overdue = "SELECT 
                    ...
                    WHERE 
                        repayments.repayment_date < CURDATE() AND DATEDIFF(CURDATE(), repayments.repayment_date) > 0 AND borrowers.loan_officer='$email'";
    ...
    $sql_overdue_amount = "SELECT SUM(repayments.amount) AS total_overdue_amount 
                           FROM repayments 
                           WHERE repayment_date < CURDATE() AND DATEDIFF(CURDATE(), repayment_date) > 0";
